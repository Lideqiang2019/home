...
function display_R0() {
    ...
    $.ajax({
        ...
        success: function(req) {
            ...
            for (var v in req) {
                var v2 = Number(v) + 1;
                source = source + "<tr><th scope='row'>" + v2 + "</th><td>" + req[v]["Country"] + "</td><td>" + req[v]["R0"] + "</td></tr>";
                xs.push(req[v]["Country"])
                ys.push(req[v]["R0"])
            }
            $("#tableHead").html("<tr><th scope='col'>Rank</th><th scope='col'>CountryName</th><th scope='col'>R0</th></tr>")
            $("#tableBody").html(source);
            display_barChart(xs, ys, 'R0 Estimate', "Country", "R0")
...
function display_peak() {
    ...
    $.ajax({
        ...
        success: function(req) {
            ...
            for (var v in req) {
                var v2 = Number(v) + 1;
                source = source + "<tr><th scope='row'>" + v2 + "</th><td>" + req[v]["Country"] + "</td><td>" + req[v]["Peak value"] + "</td></tr>";
                xs.push(req[v]["Country"])
                ys.push(req[v]["Peak value"])
            }
            $("#tableHead").html("<tr><th scope='col'>Rank</th><th scope='col'>CountryName</th><th scope='col'>Peak</th></tr>")
            $("#tableBody").html(source);

            display_barChart(xs, ys, 'Peak Value Estimate of Confirmed Cases', "Country", "Peak Value")
...
function display_time() {
    ...
    $.ajax({
        ...
        success: function(req) {
            ...
            for (var v in req) {
                var v2 = Number(v) + 1;
                source = source + "<tr><th scope='row'>" + v2 + "</th><td>" + req[v]["Country"] + "</td><td  style='width: 35%;'>" + req[v]["Peak time"] + "</td></tr>";
                xs.push(req[v]["Country"])
                ys.push(req[v]["Peak time"])
            }
            $("#tableHead").html("<tr><th scope='col'>Rank</th><th scope='col'>CountryName</th><th scope='col'>Time</th></tr>")
            $("#tableBody").html(source);
            // display_barChart(xs, ys, 'Peak Value Time Estimate of Confirmed Cases', "Country", "Peak Time")
            title_text = 'Peak Value Time Estimate of Confirmed Cases'
            yname = 'Country';
            xname = 'time';
